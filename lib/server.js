...
var Server = exports.Server = function(project, config) {
  ...
  this.listen('request', function(reqres) {
      reqres.request.cookies = reqres.response.cookies = new Cookies(reqres.request, reqres.response, this.project);

      reqres.response.listen('head', function(head) {
          reqres.response.cookies.writeHeaders(head.headers);
          return head;
        });
...
Server.prototype.start = function(callback) {
  ...
  var step = function(returned, last, next) {
    if( returned instanceof Error ) {
      sys.puts( returned.stack || returned );
    }
    else {
      next();
    }
  ...
  this.project.base_app.loadInitScripts(function() {
      self.emit('start', finished, step);
    });
};
...
Server.prototype._startRequest = function(req, res) {
  ...
  try {
    ...
    // if desired. 
    this.emit('request', this._finishRequest, this._requestStep, {request: request, response: response});
  }
  catch(err) {
...
Server.prototype._requestStep = function(returned, previous, next) {
  if( typeof returned === 'undefined' || returned === null ) {
    // then we don't do anything, a listener handled the response
  }
    ...
    this._sendError(previous.response, returned);
  }
  else {
    next(returned || previous);
  }
};
...
Server.prototype._sendError = function(response, error) {
  ...
  var step = function(returned, next) {
