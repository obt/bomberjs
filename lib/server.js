...
var Server = exports.Server = function(project, config) {
  ...
  this.listen('request', function(request, response, next) {
      request.cookies = response.cookies = new Cookies(request, response, this.project);

      response.listen('head', function(head, next) {
...
Server.prototype.start = function() {
  ...
  this.project.base_app.loadInitScripts(function() {
      self.emit('start', self._startListening, self._startStep);
    });
};

Server.prototype._startStep = function(returned, last, next) {
  if( returned[0] instanceof Error ) {
    sys.puts( returned[0].stack || returned[0] );
  }
  else {
    next();
  }
...
Server.prototype._startRequest = function(req, res) {
  ...
  try {
    ...
    // if desired. 
    this.emit('request', this._finishRequest, this._requestStep, request, response);
  }
  catch(err) {
...
Server.prototype._requestStep = function(returned, last, next) {
