...
exports.create = function(baseAppModulePath) {
  ...
  if (baseAppModulePath) {
    project.setBaseApp(baseAppModulePath);
  }

  return project;
};

function setBaseApp(modulePath) {
  this.baseApp = app.create(modulePath, this);
...
function emit(event, argument, step) {
  ...
  var observers = (this._observers ? (this._observers[event] || []) : []),
      index = 0;
