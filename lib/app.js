...
App.prototype.getAction = function(action) {
  var view = this.getView(action.view, action.app);

  if( !(action.action in view) ) {
    throw new errors.ActionNotFoundError('app: ' + (action.app || this.key) + ', view: ' + action.view + ', action: ' + action.action);
  }
