 ...
 * + `parent_raw_config`: `Object`. Optional. An app's configuration is loaded from
 *    a config.js file in its root.  This parameter will override the
 ...
 */
var App = exports.App = function(module_path, parent, parent_raw_config) {
  ...
  this._loadConfigurationAndApps(parent_raw_config);
  this._loadRouter();
...
/* App.prototype._loadConfigurationAndApps(parent_raw_config)
 *
 * Used internally to load the configuration and all subapps of this app.
 *
 * Parameters:
 *
 * + `parent_raw_config`: `Object`. The configuration from the parent app.  This
 *   will get merged with the configuration loaded from `config.js`.
 */
App.prototype._loadConfigurationAndApps = function(parent_raw_config) {
  ...
  // parent app
  var raw_config = this._inheritAndFlattenConfig(config_module.config, parent_raw_config);

  // grab the config explicitly for this app
  this.config = raw_config['.'] || {};
  delete raw_config[this.key];

  // if we have no parent_raw_config then we are the base app and the server
  // gets its settings from us.
  if( typeof parent_raw_config == 'undefined' ) {
    this.server_config = raw_config['server'] || {};
  }
  // get rid of this in all apps because it will never be used.
  delete raw_config['server']
  ...
  if( config_module.apps )
  {
    config_module.apps.forEach(function(app_module_path) {
        ...
        var app_config = this._configForApp(raw_config, app_key);
        ...
        this.apps[app_key] = new App(app_module_path, this, app_config);
      }, this);
  }
};
...
/* App.prototype._configForApp(raw_config, app_key)
 *
 ...
 * Parameters:
 *
 * + `raw_config`: `Object`. The configuration to process
 ...
 */
App.prototype._configForApp = function(raw_config, app_key) {
  var new_config = process.mixin(true, {}, raw_config);
