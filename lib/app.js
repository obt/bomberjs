...
['AppNotFoundError','ViewNotFoundError','ActionNotFoundError'].forEach(function(err_name) {
 ...
 * is an "app_path".  An app_path is a way to navigate apps to get the app
    ...
    if( !this.views[view_name] ) {
      try {
        ...
        if( err.message.indexOf('Cannot find module') === 0 ) {
          throw new errors.ViewNotFoundError('app: ' + (app_path || this.key) + ', view: ' + view_name);
        }
      }
    }
...
App.prototype.getAction = function(route) {
  // If the route's action is a function, just return that
  if( route.action instanceof Function ) {return route.action;}

  var view = this.getView(route.view, route.app);

  if( !(route.action in view) ) {
    throw new errors.ActionNotFoundError('app: ' + (route.app || this.key) + ', view: ' + route.view + ', action: ' + route.action);
  }

  return view[route.action];
};
