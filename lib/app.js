 ...
 * + `parent_config`: `Object`. Optional. An app's configuration is loaded from
 ...
 */
var App = exports.App = function(module_path, parent_config) {
  ...
  this._loadConfigurationAndApps(parent_config);
  this._loadRouter();
...
/* App.prototype._loadConfigurationAndApps(parent_config)
 *
 * Used internally to load the configuration and all subapps of this app.
 *
 * Parameters:
 *
 * + `parent_config`: `Object`. The configuration from the parent app.  This
 *   will get merged with the configuration loaded from `config.js`.
 */
App.prototype._loadConfigurationAndApps = function(parent_config) {
  ...
  // parent app
  var config = this._inheritAndFlattenConfig(config_module.config, parent_config);

  // grab the config explicitly for this app
  this.config = config['.'] || {};
  delete config[this.key];

  // if we have no parent_config then we are the base app and the server
  // gets its settings from us.
  if( typeof parent_config == 'undefined' ) {
    this.server_config = config['server'] || {};
  }
  ...
  if( config_module.apps )
  {
    config_module.apps.forEach(function(app_module_path) {
        ...
        var app_config = this._configForApp(config, app_key);
        ...
        this.apps[app_key] = new App(app_module_path, app_config);
      }, this);
  }
};
...
/* App.prototype._configForApp(config, app_key)
 *
 ...
 * Parameters:
 *
 * + `config`: `Object`. The configuration to process
 ...
 */
App.prototype._configForApp = function(config, app_key) {
  var new_config = process.mixin(true, {}, config);
