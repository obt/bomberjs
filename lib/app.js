...
var App = exports.App = function(module_path, project_scope, parent, parent_apps_config) {
  ...
  this.path = utils.require_resolve(this.module_path, false);
...
// this is async, so we take a callback for calling when we are done.
App.prototype.loadInitScripts = function(serverCallback) {
  var self = this;

  var callback = function() {
    process.fs.readdir(path.join(self.path,'init'), function(error, dir) {
        if( error ) {
          ...
          dir.forEach(function(file) {
              if( file.match(/^[^.]+\.js$/) ) {
                var load = require(path.join(self.path, 'init',path.basename(file,'.js'))).load;
                if( load ) {
                  inits.push(load);
                }
              }
          ...
          var loadNextInit = function() {
            if( inits.length < 1 ) {
              return serverCallback();
            }

            var load = inits.shift();
            if( load.length == 3 ) {
              load(self, self.project, loadNextInit);
            }
            else {
              load(self, self.project);
              loadNextInit();
            }
          }
  ...
  var loadScriptsForNextApp = function() {
    if( appKeys.length < 1 ) {
      return callback();
    }

    self.apps[appKeys.shift()].loadInitScripts(loadScriptsForNextApp);
  }

  loadScriptsForNextApp();
};
 ...
 * Throws:
 *
 * + If there is no file corresponding to `view_name`
 * + If there is no app corresponding to `app_path`
 */
    ...
    if( !this.views[view_name] ) {
      try {
        ...
        if( err.message.indexOf('Cannot find module') === 0 ) {
          throw new new Error('View Not Found. App: ' + (app_path || this.key) + ' View: ' + view_name);
        }
        throw err;
      }
    }
 ...
 * Throws:
 *
 * + If it can't find a function specified by `route_action`
 *
...
App.prototype.getAction = function(route_action) {
  ...
  if( !(route_action.action in view) ) {
    throw new Error('Action Not Found. App: '
...
App.prototype._loadConfigurationAndApps = function(parent_apps_config) {
  ...
  this.project.config = process.mixin(true, process.mixin(true, {}, config_module.project_config), this.project.config);
...
App.prototype._configForApp = function(apps_config, app_key) {
  ...
  for( var key in new_config ) {
    if( key.charAt(0) === '.' && this._parseAppPath(key)[0] !== app_key ) {
