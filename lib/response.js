 ...
 * Parameters:
 *
 * + `res`: a `http.ServerResponse` instance.
 */
var Response = exports.Response = function(res) {
  this._response = res;
  this.headers = {};
};
makeChainable(Response);
 ...
 * 
 * The default encoding used for `http.serverResponse.write(chunk, encoding)`
 */
 ...
 *
 * The encoding used for `http.serverResponse.write(chunk, encoding)`
 */
 ...
 *
 * Used to fill in the Content-Type header if it isn't set.  If this is
 * `null`, `mime.charsets.lookup()` is called with
...
Response.prototype.finish = function() {
  ...
  return this.emit('finished', this._finishFinish, null);
};

Response.prototype._finishFinish = function(str) {
  if( str ) {
    this._response.write(str, this.transferEncoding);
  }
  this._finished = true;
...
Response.prototype.sendHeaders = function() {
  ...
  if( !('Content-Type' in this.headers) ) {
    ...
    this.charset = typeof this.charset !== 'undefined' ? this.charset : mime.charsets.lookup(this.mimeType);
    this.headers['Content-Type'] = this.mimeType + (this.charset ? '; charset=' + this.charset : '');
  }

  return this.emit('head', this._finishSendHeaders, null, {status: this.status, headers: this.headers});
};

Response.prototype._finishSendHeaders = function(head) {
  ...
  this._response.sendHeader(head.status, head.headers);
};

/* Response.prototype.send(str)
 *
 * Very similar to `http.ServerResponse.write` except it handles sending the
...
Response.prototype.send = function(str) {
  ...
  return this.emit('body', this._finishSend, this._sendStep, str);
};

Response.prototype._sendStep = function(str, next) {
  if( typeof str === 'undefined' || str === null ) {
    // a listener for the 'body' event, didn't return anything, indicating
    // they want to cancel the chain.
  }
  next(str);
};

Response.prototype._finishSend = function(str) {
  this._response.write(str, this.transferEncoding);
 ...
 * returned Promise will also emit the same errors as `fs.open` and
 * `fs.read`.
 */
Response.prototype.sendFile = function(filename) {
  ...
  return promise.execute(fs.open, filename, process.O_RDONLY , 0666)
    .then(function(fd) {
      var pos = 0;
      ...
      var callback = function(args) {
        ...
        if( bytesRead < Response.__bytesToRead ) {
          ...
          return promise.execute(fs.read, fd, Response.__bytesToRead, pos, self.transferEncoding).then(callback);
        }
      };
