...
Response.prototype.sendFile = function(filename) {
  ...
  fs.open(filename, process.O_RDONLY , 0666, function(error, fd) {
      if( error ) {
      ...
      var callback = function(error, chunk, bytesRead) {
        if( error ) {
          return errback(error);
        }
        ...
        if( bytesRead < Response.__bytesToRead ) {
          ...
          fs.read(fd, Response.__bytesToRead, pos, self.transferEncoding, callback);
        }
      };
