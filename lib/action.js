...
exports.processAction = function(request, response, action) {
  ...
  if( action_response instanceof process.Promise || action_response instanceof Promise ) {
    ...
    action_response.addErrback(complete_handler);
  }
  else {
...
function err(err) {
  if( err instanceof HTTPResponse ) {
    ...
    this.response._response.sendHeader(500, {'Content-Type': 'text/plain'});
    this.response._response.sendBody('some other error');
    this.response._response.finish();
  }
}
