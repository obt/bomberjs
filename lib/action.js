 ...
 * 3.  The action returned something else.  In this case we want to write this to
 *     the client.  If it is a string, write it as it is, and if it is anything 
 *     else convert it to a JSON string and then write it.
 *
 * Parameters:
 *
...
function complete(action_response) {
  if( typeof action_response == "undefined" ) {
    ...
    if(action_response.constructor == String) { // return text/html response
      ...
      this.response.send(JSON.stringify(action_response));
    }
  }
};
